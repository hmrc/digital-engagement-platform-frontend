...
describe("When loading a page and waiting for changes", () => {
    ...
    beforeEach(() => {
        ...
        elementWatcherMock = jest
            .spyOn(elementWatcher, 'waitForEl')
    ...
    describe("If there is a timeout", () => {
        ...
        <div class="hide-text-on-error"></div>`;
            ...
            const timeoutFunction = elementWatcherMock.mock.calls[0][2];
            timeoutFunction();

            expect(document.getElementById('HMRC_Fixed_1').textContent).toEqual(
                "Sorry, there’s a problem with this service. Refresh the page to try again."
            );
            expect(
                document.getElementsByClassName('hide-text-on-error')[0].style.display
            ).toBe('none');
        });
        ...
        it("will report technical difficulties if timeout for virtual assistant", () => {
            window.history.replaceState({}, '', '/ask-hmrc/virtual-assistant/some-page');
