...
class CiapiControllerSpec
  ...
  "Nuance Full Page CIAPI Test Controller" should {
    ...
    "render trusts webchat page if shutter flag is true" in {
      val application = builder.configure("features.digitalAssistants.showTrusts" -> "true").build()

      running(application) {
        val request = FakeRequest(GET, routes.CiapiController.trusts.url)
        val result = route(application, request).get
        val doc = asDocument(contentAsString(result))
        status(result) mustBe OK
        doc.select("h1").text() mustBe "Ask HMRC online"
    ...
    "render trusts webchat page is not displayed if shutter flag is false. 404 page received" in {
      val application = builder.configure("features.digitalAssistants.showTrusts" -> "false").build()
    ...
    "render Get help from HMRC DA page if shutter flag is true" in {
      val application = builder.configure("features.digitalAssistants.showGetHelpFromHmrcDa" -> "true").build()

      running(application) {
        val request = FakeRequest(GET, routes.CiapiController.getHelpFromHmrcDa.url)
        val result = route(application, request).get
        val doc = asDocument(contentAsString(result))
        status(result) mustBe OK
        doc.select("h1").text() mustBe "Ask HMRC online"
    ...
    "Get help from HMRC DA page is not displayed if shutter flag is false. 404 page received" in {
      val application = builder.configure("features.digitalAssistants.showGetHelpFromHmrcDa" -> "false").build()
