    ...
    technicalError: function () {
        ...
        if (errorEle) {
            if (errorEle.classList.contains("chat-in-progress")) {
                errorEle.innerHTML = "<p class='cui-technical-error error-message' tabindex='0' aria-live='assertive' role='alert'> You've already started a chat in another tab or window. Return to the chat to continue your conversation. </p>";
            } else {
                errorEle.innerHTML = '<p class="cui-technical-error error-message" tabindex="0" aria-live="assertive" role="alert">Sorry, there’s a problem with this service. Refresh the page to try again.</p>';
            }
        }
